...
class solution():
    ...
    # 得到每个大维度下的小维度分数之和
    def slip_sum_grades(self,file_every_index, grades_dict):
        all_index = []
        every_6_index = []
        f8_grade = []
        f8_sum_list = []
...
questions=open(r"./question", "r")
file_re_index = open(r"./re-index", "r")
file_every_index = open(r"./all-index", "r")
s=solution()
grades = s.show_question(questions) # 得到所有输入的分数
grades = s.alculate_reverse_score(file_re_index, grades) # 把反向的分数取负
grades_dict = s.grade_covdict(grades)  # 分数转化为字典格式{key=index：value=grades}
f8_sum_list = s.slip_sum_grades(file_every_index, grades_dict)  # 得到每个大维度
all_grades = s.add_all_grades(f8_sum_list)  # 得到全部的分数[N,E,O,A,C,N1~N6,E1~E6,…………]
really_grade = s.calculation_really_grades(all_grades)  # 根据计算法则得出最后的分数
s.print_grades(really_grade)  # 打印分数
